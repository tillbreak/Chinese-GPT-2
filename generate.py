...
# --- Post-generation sanitization (默认开启) ---
BAD_KEYWORDS = [
    '小说网', '网址', '手机阅读', '收藏本站', '下载地址', '点击此处', '全文阅读', '推荐票', '书友',
    '顶点', '笔趣阁', '起点中文网', '手机版', '本书来自', '本章未完', '继续阅读', '更新最快', 'TXT', '小说阅读网'
]
URL_RE = re.compile(r'(https?://\S+|www\.[\w.-]+|[\w-]+\.(com|cn|net|org|info|cc|tv|me|top|xyz)(/\S*)?)', re.IGNORECASE)
BRACKET_SITE_RE = re.compile(r'『[^』\n]*?(小说网|网址)[^』\n]*』')
SQUARE_AD_RE = re.compile(r'【[^】\n]*?(小说网|网址)[^】\n]*】')

def sanitize_text(text: str) -> str:
    # 去除括注广告
    text = BRACKET_SITE_RE.sub('', text)
    text = SQUARE_AD_RE.sub('', text)
    # 行级处理：去 URL，过滤含站点词的整行
    lines = []
    for ln in text.splitlines():
        ln2 = URL_RE.sub('', ln)
        if any(k in ln2 for k in BAD_KEYWORDS):
            continue
        lines.append(ln2)
    text = '\n'.join(lines)
    # 折叠多余空行（>=3 折叠为 2）
...
def collapse_newlines(text: str, max_newlines: int = 1) -> str:
    """将连续换行折叠为最多 max_newlines 个"""
    if max_newlines < 1:
        max_newlines = 1
...
def main():
    ...
    parser.add_argument('--nsamples', default=3, type=int, required=False, help='生成几个样本')
    ...
    parser.add_argument('--prefix', default='外星人', type=str, required=False, help='生成文章的开头')
    ...
    parser.add_argument('--no_sanitize', action='store_true', help='禁用输出净化（默认开启，移除URL/广告）')
    parser.add_argument('--max_newlines', default=1, type=int, required=False, help='同一位置最多保留的连续换行数')
    ...
    while True:
        ...
        for _ in range(nsamples // batch_size):
            ...
            for i in range(batch_size):
                ...
                # 折叠连续换行至最多 args.max_newlines 个
                text = collapse_newlines(text, args.max_newlines)
                print(text)
                if args.save_samples:
                    safe_text = text
                    samples_file.write(info)
                    samples_file.write(safe_text)
                    samples_file.write('\n')
